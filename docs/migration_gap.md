...
- 项目中的菜单、表格列、表单标签、按钮、确认提示、控制台说明等文字已抽到后台“文字配置”，支持 `/api/text-configs/` 统一维护，前端启动读取 `/api/ui-text/` 覆盖默认文案。
...
- 用户体系：TgUser、用户排行、用户机器人归属已基础覆盖；用户拉黑/取消拉黑、置顶/取消置顶后台操作已迁入；会员订单开通会回写用户会员等级和到期时间。
- 能量业务细分：小时套餐、小时价格、笔数套餐、笔数地址、智能托管、智能托管地址、闪租入口、代理订单、佣金、预支记录已基础覆盖；能量订单支付/委托中/成功/失败、预支审核、SOHU 委托演练/真实开关已迁入。
- 兑换业务细分：兑换配置、兑换订单、兑换记录、黑名单已基础覆盖；兑换订单/记录已支持已支付、已出款、失败、取消等后台流转；USDT 链上监听扫描、交易入库、会员充值/兑换订单自动匹配已接入；自动出款和更严格风控匹配待深化。
- 会员业务细分：会员商品、会员订单、会员充值、会员活动、会员佣金已覆盖；会员订单已支持已支付/开通权益/取消，会员充值可确认到账并写余额流水，会员佣金可按邀请人结算到余额。
- 财务业务：余额流水、提现审核、充值配置、监听地址已基础覆盖；余额手工调账、提现通过/拒绝/已打款、提现打款演练已迁入；监听地址扫描可自动确认充值并写流水；真实链上打款保留 WITHDRAWAL_PAYOUT_ENABLED/私钥保护开关，待接入多签策略。
- 机器人业务：多机器人配置、群组绑定、自动回复、按钮、Webhook 用户/群组识别、/start 邀请关系、群发演练、群发记录已覆盖；真实群发/收益播报需开启 TELEGRAM_SEND_ENABLED 并接入生产域名。
- 统计报表：用户、订单、收益、佣金、兑换、能量租赁多维统计已迁入控制台和 `/api/reports/overview/`，支持 days 与 bot_id 查询过滤；更复杂图表可后续按运营需求扩展。
